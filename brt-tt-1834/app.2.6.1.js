var Slider = function() {
	...
	function _init() {
		...
		if(!_withinMinAndMax())
		{
			_debug("not within configured min/max screen sizes, hiding Slider...");
			displayAd = false;
			_registerEvent({
				id: options.id,
				pid: options.pid,
				trid: options.trid,
				sessionId: options.sessionId,
				action: "not displayed due to configured min/max values"
			});
		}
	...
	function _withinMinAndMax(){
		
		var screenWidth = $(window).width();
		_debug('screenwidth', screenWidth);
		if(screenWidth >= options.minScreenWidth && screenWidth <= options.maxScreenWidth)
		{
			return true;
		}
		else if(options.minScreenWidth == 0 && options.maxScreenWidth == 0)
		{
			return true;
		}
		else
